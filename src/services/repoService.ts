...
export class RepoService {
    async getOrganizationRepos(scmId: string, orgId: string): Promise<Repository[]> {
    ...
    async createWebhook(repoLocator: RepoLocator): Promise<string> {
        ...
        const response = await httpClient.post(UrlPaths.webhooks.create, config).then((r) => r.json());
        return response;
    }
    ...
    async removeWebhook(repoLocator: RepoLocator) {
        ...
        return httpClient.request(UrlPaths.webhooks.remove, config);
    }
}
